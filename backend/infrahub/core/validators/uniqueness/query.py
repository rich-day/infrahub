...
class NodeUniqueAttributeConstraintQuery(Query):
    ...
    async def query_init(self, db: InfrahubDatabase, **kwargs: Any) -> None:  # noqa: ARG002, PLR0915
        ...
        MATCH rel_path = (attr_start_node:%(node_kind)s)-[:IS_RELATED]-(relationship_node:Relationship)-[:IS_RELATED]-(related_n:Node)-[:HAS_ATTRIBUTE]->(rel_attr:Attribute)-[:HAS_VALUE]->(rel_attr_value:AttributeValue)
        ...
        MATCH rel_path = (attr_start_node:%(node_kind)s)-[:IS_RELATED]-(relationship_node:Relationship)-[:IS_RELATED]-(related_n:Node)
