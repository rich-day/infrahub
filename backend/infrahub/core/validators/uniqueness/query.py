...
class NodeUniqueAttributeConstraintQuery(Query):
    ...
    async def query_init(self, db: InfrahubDatabase, **kwargs: Any) -> None:  # noqa: ARG002,PLR0915
        ...
        OPTIONAL MATCH rel_path_with_value = (attr_start_node:%(node_kind)s)-[:IS_RELATED]-(relationship_node:Relationship)-[:IS_RELATED]-(related_n:Node)-[:HAS_ATTRIBUTE]->(rel_attr:Attribute)-[:HAS_VALUE]->(rel_attr_value:AttributeValue)
        ...
        OPTIONAL MATCH rel_only_attr_path = (attr_start_node:%(node_kind)s)-[:IS_RELATED]-(relationship_node:Relationship)-[:IS_RELATED]-(related_n:Node)
        ...
        WITH attr_start_node, rel_path_with_value, relationship_node, related_n, rel_attr, rel_attr_value
        RETURN attr_start_node AS start_node, rel_path_with_value AS potential_path, relationship_node.name as rel_identifier, rel_attr.name as potential_attr, rel_attr_value.value as potential_attr_value
        """

        relationship_only_attr_paths_filter_subquery = """
        WITH attr_start_node, rel_only_attr_path, relationship_node, related_n
        RETURN attr_start_node AS start_node, rel_only_attr_path as potential_path, relationship_node.name as rel_identifier, "id" as potential_attr, related_n.uuid as potential_attr_value
        """
        ...
        if attr_paths_with_value:
            ...
            returned_attributes.extend(["attr_path_with_value", "attr", "attr_value"])
        ...
        if relationship_attr_paths_with_value:
            ...
            returned_attributes.extend(["rel_path_with_value", "relationship_node", "related_n", "rel_attr", "rel_attr_value"])
        if relationship_only_attr_paths:
            ...
            returned_attributes.extend(["rel_only_attr_path", "relationship_node", "related_n"])
